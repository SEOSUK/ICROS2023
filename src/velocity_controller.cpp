...
Eigen::Matrix2d Scaled_Jacobian;
...
void jointcallback(const sensor_msgs::JointState::ConstPtr &msg)
{
	...
	measured_angle[1] = msg->position.at(1);   //부호가 왜 반대냐 ......
	measured_velocity[0] = msg->velocity.at(0);
	measured_velocity[1] = msg->velocity.at(1);
}
...
int main(int argc, char** argv)
{
	...
	ros::Publisher anglePub = n.advertise<geometry_msgs::Twist>("angle_Pub", 100); 
...
//		Jacobian_inverse = solve_Jacobian_Inverse(measured_angle[0], measured_angle[1], Link1, Link2);
	...
	std::cout << Jacobian_transpose << std::endl;
	std::cout << "-------------------" << std::endl;
	std::cout << Scaled_Jacobian << std::endl;
	std::cout << "==================\n==================\n";
	...
	//---------------포지션 에러 구하기 ---------------//
		...
		Scaled_Jacobian = (Jacobian_transpose * Jacobian).inverse() * Jacobian_transpose;

		angular_velocity_command = Jacobian_transpose * position_gain;
		...
		angle_pub.linear.x = angular_velocity_command[0];
		angle_pub.linear.y = angular_velocity_command[1];
		angle_pub.angular.x = measured_velocity[0];
		angle_pub.angular.y = measured_velocity[1];




		forPub.publish(command);
		anglePub.publish(angle_pub);
		ros::spinOnce();
		rate.sleep();
